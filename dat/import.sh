#!/bin/bash
# -------------------------------------------------------------------------
# self-extract & import script automatically generated by ctenv
#
# depending on user's preference, this script can
#   a) generate a patch file
#   b) create a directory with source files exported by ctenv
#   c) be used to import exported changes into a given clearcase's view
#
...
CTENV_GCONST_IMPORT_LATEST_DIR="pred"
CTENV_GCONST_IMPORT_CHANGE_DIR="change"
CTENV_GCONST_MOBILE_GEN_PATCH="PATCH"
CTENV_GCONST_MOBILE_GEN_ZIP="ZIP"
...
function ctenv_self_extract
{
    ...
    if [ -d ${tgtDir} ]; then
        ...
        mkdir -p ${tgtDir}/${CTENV_GCONST_IMPORT_CHANGE_DIR}
        mkdir -p ${tgtDir}/${CTENV_GCONST_IMPORT_LATEST_DIR}
    fi

    local CTENV_ARCHIVE_START=`awk '/^__CTENV_TAR_ARCHIVE_START__/ {print NR + 1; exit 0; }' ${srcFile}`
    local CTENV_ARCHIVE_START2=`awk '/^__CTENV_TAR_ARCHIVE_START2__/ {print NR + 1; exit 0; }' ${srcFile}`
    local CTENV_ARCHIVE_END=$((CTENV_ARCHIVE_START2 - 2))

    local -a importFileList=(`tail -n+${CTENV_ARCHIVE_START2} ${srcFile} | tar tjf -`)
    ...
    tail -n+${CTENV_ARCHIVE_START2} ${srcFile} | \
        tar -C ${tgtDir}/${CTENV_GCONST_IMPORT_CHANGE_DIR} -xvjf - | \
        grep -v ${CTENV_GCONST_CC_LIST_FILE} | \
        xargs -I {} printf "\t%s\n" {}

    sed -n "${CTENV_ARCHIVE_START},${CTENV_ARCHIVE_END}p;${CTENV_ARCHIVE_END}q" ${srcFile} \
        | tar -C ${tgtDir}/${CTENV_GCONST_IMPORT_LATEST_DIR} -xvjf - &> /dev/null

    mv -f ${tgtDir}/${CTENV_GCONST_IMPORT_LATEST_DIR}/vobs/${CTENV_GCONST_CC_LIST_FILE} ${tgtDir} \
        &> /dev/null
...
function ctenv_gen_file
{
    ...
    if [ ${genMode} = ${CTENV_GCONST_MOBILE_GEN_PATCH} ]; then
        ctenv_prt_info ${CTENV_GCONST_MOBILE_SH} \
            "generating a patch"

        if [ ${tgtFile} = "." ]; then
            tgtFile="${CTENV_GCONST_MOBILE_SH_NAME%.*}.patch"
        fi
    else
        ctenv_prt_info ${CTENV_GCONST_MOBILE_SH} \
            "generating a zip"
    ...
    cd ${tmpDir}
    if [ ${genMode} = ${CTENV_GCONST_MOBILE_GEN_PATCH} ]; then
        diff -rupNbB ${CTENV_GCONST_IMPORT_LATEST_DIR} ${CTENV_GCONST_IMPORT_CHANGE_DIR} \
            >| ${tmpFile}
    else
        rm -f ${tmpFile}
        zip -9 -r ${tmpFile} ${CTENV_GCONST_IMPORT_LATEST_DIR} ${CTENV_GCONST_IMPORT_CHANGE_DIR}
    fi
    cd - &> /dev/null
...
function ctenv_prt_usage
{
    ...
    echo "    -p [PATCH_FILE]"
    echo "       create a patch file, [PATCH_FILE]"
    echo "    -z [ZIP_FILE]"
...
while [ $# -gt 0 ]; do
	case "$1" in
        ...
        -p)
            ...
            ctenv_gen_file `ctenv_readlink -f $0` $2 ${CTENV_GCONST_MOBILE_GEN_PATCH}
            shift
            ;;
        -z)
            if [ -z ${2:-} ]; then
            ...
            ctenv_gen_file `ctenv_readlink -f $0` $2 ${CTENV_GCONST_MOBILE_GEN_ZIP}
            shift
            ;;
        *)
            ctenv_prt_usage
